...
def returnTweetsBasedOnSearchModel(article_id, searchModel):
    #accepts a search model, article title, and article id	
...
class CosineSimilarity:
	...
	def query(self, query_id, query_text, return_size=40):
		...
		term_doc = self.vectorizer.fit_transform([query_text]+list(self.tweets.tweet)) #returns document term matrix
		print (self.vectorizer.vocabulary_)
		ind = [query_text] + list(self.tweets.tweet)
...
cosine_similarity_obj.query(sample_query_id, sample_query_text)
