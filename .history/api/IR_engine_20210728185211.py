...
class CosineSimilarity:
	...
	def __init__(self, data, documents, type='tfidf'):
		self.data = data 
	...
	def query(self, query_id, query_text, return_size=40):
		...
		term_doc = self.vec.fit_transform([query_text]+list(self.data.clean_titles))
	...
	def change_matrix_type(self, type):
		if type == 'tfidf':
			...
			self.vec = CountVectorizer() #transforms the entire word matrix into a set of vectors
		else:
...
tweets_data = dataProcessor.tweets_data
titles_data = dataProcessor.titles_data

sample_q_id = "f7ca322d-c3e8-40d2-841f-9d7250ac72ca"
sample_q_text = "Worcester breakfast club for veterans gives hunger its marching orders"

cosine_similarity = CosineSimilarity(data = titles_data, documents = tweets_data)
