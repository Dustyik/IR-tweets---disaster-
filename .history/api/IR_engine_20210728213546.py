...
class CosineSimilarity:
	...
	def query(self, query_id, query_text, return_size=40):
		...
		term_doc = self.vectorizer.fit_transform([query_text]+list(self.data.clean_titles))
		ind = ['query'] + list(self.documents)
		self.matrix = pd.DataFrame(term_doc.toarray(), columns=self.tweets.tweet, index=ind)
...
print (cosine_similarity.vec.fit_transform([sample_query_text]))
