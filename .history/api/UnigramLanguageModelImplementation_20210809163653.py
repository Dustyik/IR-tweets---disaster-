...
class UnigramLanguageModel:
    ...
    def calculate_interpolated_sentence_probability(self, querySentence:list, document, alpha=ALPHA, normalize_probability=NORMALIZE_PROBABILITY):
        total_score = 1
        documentWordFrequencyDictionary = self.create_words_frequency_dict(document, collection = False)
    ...
    def getQueryLikelihoodModelScore(self, querySentence:list):
        self.tweets_data["QueryLikelihoodModelScore"] = self.tweets_data.apply(lambda row: self.calculate_interpolated_sentence_probability(querySentence, row.clean_text), axis = 1d)
