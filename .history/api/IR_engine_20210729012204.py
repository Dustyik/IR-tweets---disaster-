...
class DataProcessor:
	def __init__(self):
		...
		self.cosineSimilarity = EuclideanDistance(self.titles_data, self.tweets_data)
...
class EuclideanDistance:
	...
	def get_result(self, return_size):
		...
		self.result = []
		for i in top_id:
			filt = self.data[self.data.tweet==i]
			for ind, r in filt.iterrows():
				rel = r['rel']
				text = r['text']
				related = r['topic']
	...
	def query(self, query_id, query_text, return_size=10):
		self.query_id = query_id
		term_doc = self.vec.fit_transform([query_text]+list(self.tweets.clean_text))
		ind = ['query'] + list(self.tweets.tweet)
