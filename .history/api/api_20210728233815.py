...
def getTitles():
    try:
        ...
        data = dataProcessor.titles_data["title"]
        data = data.to_json()
        js = json.dumps(data)
        # delete the stl file after done parsing
        response = Response(js, status=200, mimetype='application/json')
...
def applySearchModels():
    try:
        ...
        #print (jsonData)
        articleId = getArticleId(dataProcessor = dataProcessor, articleTitle = jsonData["queryTitle"])
        searchModel = jsonData["data"]
        data = returnTweetsBasedOnSearchModel(dataProcessor = dataProcessor, articleId = articleId, searchModel = searchModel)
        print (data)
        data = "Success"
        js = json.dumps(data)
        # delete the stl file after done parsing
