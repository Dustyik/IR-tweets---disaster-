...
const SEARCH_MODELS = ["Tf-idf w Cosine Sim", "Tf-idf w Euclidean Dist", "Okapi-BM25", "Word2Vec w Cosine Similarity", "W2Ved": "Word2Vec w Euclidean Distance"]
...
export const Sidebar = () => {
    ...
    const {setSearchModels, searchModels, title, removeAllTweets, addTweet, addRankingScore, getRankingScore} = useContext(GlobalContext);
    ...
    const applyFilter = () => {
        API.applySearchFilters(searchModels, title.title, title.id).then(
            res => {
                removeAllTweets()
                const data = JSON.parse(res.data.pandas_data)
                const ranking = res.data.ranking_score
                addRankingScore(ranking)

    

                console.log(data)

                const relevanceData = data.relevance_score
                const tweetData = data.tweet
                const tweetIdData = data.id
                const relatedArticlesData = data.article_id
