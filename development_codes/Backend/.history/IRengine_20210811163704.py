...
from PairwiseLTR import *
import time

RETURN_SIZE = 20 #Parameter that controls how many tweets to return to the front end

titles_file_path = "./csv_data/article_titles.csv"
tweets_file_path = "./csv_data/tweets_data_stemmed.csv"
...
class DataProcessor:
	def __init__(self):
		...
		self.cosineSimilarity = CosineSimilarity(self.tweets_data)
		self.euclideanDistance = EuclideanDistance(self.tweets_data)	
		...
		self.pairwiseLTR = PairwiseLTR(self.tweets_data, self.titles_data)
		self.titles_data = self.pairwiseLTR.return_test_articles() # Because of pariwise LTR, article titles have to be split into train and test 
	...
	def returnTweetsBasedOnSearchModel(self, articleId, articleTitle, searchModel):
		...
		if searchModel == SEARCH_MODELS["tfcs"]:
			rankedDocs = self.cosineSimilarity.query(articleId, articleTitle)[:RETURN_SIZE]
		if searchModel == SEARCH_MODELS["tfed"]:
			rankedDocs = self.euclideanDistance.query(articleId, articleTitle)[:RETURN_SIZE]
		...
		'''
		if searchModel == SEARCH_MODELS["LTR"]:
			rankedDocs = self.pairwiseLearningToRankQuery(articleId, articleTitle)
	...
	def pairwiseLearningToRankQuery(self, articleId, articleTitle):
		rankedDocs = self.pairwiseLTR.rank_docs(articleId, articleTitle)
	...
	def average_model_performance(self, frac = 1):
		#evaluate the performance of each model 50 randomly selected tweets
		#when using this function, LTR must be commented out
		test_data = self.titles_data
		test_data = test_data.sample(frac=frac).reset_index(drop=True)
		ndcg_results_dictionary = {}
		time_results_dictionary = {}
		for key in SEARCH_MODELS:
			ndcg_results_dictionary[key] = []
			time_results_dictionary[key] = []


		for index, row in test_data.iterrows():
				articleTitle = row.title
				articleId = row.id
				for search_model_key in SEARCH_MODELS:
					searchModel = SEARCH_MODELS[search_model_key]
					start = time.time()
					ranked_docs = self.returnTweetsBasedOnSearchModel(articleId = articleId, articleTitle = articleTitle, searchModel = searchModel)
					end = time.time()
					rankingScore = get_rank(RANKING_MODEL, ranked_docs)
					ndcg_results_dictionary[search_model_key].append(rankingScore)
					time_results_dictionary[search_model_key].append(end-start)
					#print (SEARCH_MODELS[search_model_key], rankingScore)
		
		for key in ndcg_results_dictionary:
			ndcg_results_dictionary[key] = np.mean(ndcg_results_dictionary[key])
			time_results_dictionary[key] = np.mean(time_results_dictionary[key])
		return ndcg_results_dictionary, time_results_dictionary
...
dataProcessor = DataProcessor()
ndcg_result, time_result = dataProcessor.average_model_performance()
#result = dataProcessor.returnTweetsBasedOnSearchModel(test_title_1_id, test_title_1, "Pairwise LTR")
print (ndcg_result)
print (time_result)
