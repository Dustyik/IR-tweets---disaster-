...
ALPHA = 0.75 #Hyper parameter controlling alpha
...
class QueryLikelihoodModel:
    def __init__(self, tweets_data): 
        ...
        self.longest_tweet_length = self.calculate_longest_tweet(self.tweets_data)
    ...
    def calculate_longest_tweet(self, tweets_data):
        longestTweet = 0
        for index, row in tweets_data.iterrows():
            tweetLength = len(list(row.clean_text.split(" ")))
            if tweetLength > longestTweet:
    ...
    def calculate_unigram_probability(self, word: str, wordCollectionFrequencyDictionary, collection = False):
        if collection:
            totalNumberOfWords = self.calculate_total_no_of_words(wordCollectionFrequencyDictionary)
        else:
            totalNumberOfWords = self.longest_tweet_length #to ensure that for shorter tweets, dividing by a smaller tweet length doesnt give it an advantage
        try:
    ...
    def calculate_interpolated_sentence_probability(self, querySentenceList:list, document, alpha=ALPHA, normalize_probability=NORMALIZE_PROBABILITY):
        ...
        for word in querySentenceList:
            score_of_word = alpha*(self.calculate_unigram_probability(word, documentWordFrequencyDictionary)) + (1 - alpha)*(self.calculate_unigram_probability(word, self.wordsCollectionFrequencyDictionary, collection=True))
