...
class DataProcessor:
	def __init__(self):
		...
		self.titles_data = self.pairwiseLearningToRankQuery.return_test_articles()
	...
	def average_model_performance(self, frac = 0.5):
		...
		for index, row in test_data.iterrows():
				...
				for search_model_key in SEARCH_MODELS:
					...
					results_dictionary[search_model_key].append(rankingScore)
					#print (SEARCH_MODELS[search_model_key], rankingScore)
		
		for key in results_dictionary:
			results_dictionary[key] = np.mean(results_dictionary[key])
