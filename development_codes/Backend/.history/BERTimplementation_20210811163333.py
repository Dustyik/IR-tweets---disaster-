...
K = 100 #This parmeter Controls the number of
BERT_MODEL = "paraphrase-multilingual-mpnet-base-v2" #pre-trained BERT model that we are using
...
class BERTmodel:
    ...
    def return_BERT_query(self, article_id, article_title):
        topKResults = self.tfidf_retrieve_K_tweets(article_id, article_title) #100 Tweets are first retrieved with tf-idf
