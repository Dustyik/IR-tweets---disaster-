...
def returnTweetsBasedOnSearchModel(dataProcessor, articleId, articleTitle, searchModel):
	...
	rankedDocs = rankedDocs.apply(lambda row:checkIfArticleIdMatchesQueryId(row), articleId)
	print (rankedDocs)
	return rankedDocs
...
dataProcessor = DataProcessor()
ret = returnTweetsBasedOnSearchModel(dataProcessor, "testId", test_title_1_id, "Word2Vec w Cosine Similarity")
display(ret)