...
class PairwiseLTR:
    ...
    def __init__(self, tweets_data, titles_data, max_doc_len=50, max_query_len=50, batch_size=128, method="trim", data_path="../tweets_data_stemmed.csv", article_titles_path="../article_titles.csv"):
        #self.data = pd.read_csv(data_path)
        #self.data = self.data.replace({"relevance_score":2}, 1)
        #self.titles = pd.read_csv(article_titles_path)
        self.data = tweets_data
        self.titles = titles_data
        self.articles_id = list(self.data.article_id.unique())
        self.get_pos_neg(method)
        self.init_model_parameters(max_doc_len, max_query_len)
    ...
    def init_model_parameters(self, max_doc_len, max_query_len):
        ...
        self.train_articles, self.test_articles = train_test_split(self.articles_id, test_size=1/10, random_state=179)
        dataset = []
    ...
    def rank_docs(self, qry, doc_list):
        doc_list = self.tweets_data.tweet.to_list()
        scores = []
        for doc in doc_list:
            self.model.eval()
            with torch.no_grad():
        ...
        for doc, score in scores:
            qry_id = self.titles[self.titles["title"]==qry]["id"].to_list()[0]
            article_filt = self.data[self.data["article_id"]==qry_id]
            doc_id = self.data[self.data["tweet"]==doc]["tweet_id"].to_list()[0]
            doc_filt = article_filt[article_filt["tweet_id"]==doc_id]
            if doc_id not in results["tweet_id"].to_list():
