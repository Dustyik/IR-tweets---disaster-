...
class Word2VecModel:
    ...
    def return_most_significant_tweets(self, query):
        ...
        self.tweets_data['similarity']=self.tweets_data['vector'].apply(lambda x: cosine_similarity(np.array(        query_vector_embedding = self.document_embedding_w2v(query_vector)
).reshape(1, -1),np.array(x).reshape(1, -1)).item())
