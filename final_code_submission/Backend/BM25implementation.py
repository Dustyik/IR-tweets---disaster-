...
class BM25Class:
	...
	def __init__(self, files, query):
		self.tf = {}
		self.df = {}
		self.files = files #pandas dataframe of tweets
		self.file_to_terms = self.process_files() #dictionary of key tweetid, value tokenised tweet
	...
	def process_files(self):
		...
		for index, row in self.files.iterrows():
			file_to_terms[row.tweet_id] = row.clean_text #read the whole text of a file into a single string with lowercase
	...
	def regular_index(self,termlists):
		...
		for filename in termlists.keys(): #iterates through all the tweetids
	...
	def docLen(self):
		...
		'''
		dl = {} #dictionary of document length
		for file in self.file_to_terms:
	...
	def BM25scores(self,qlist):
		...
		'''
		total_score = {}
		for doc in self.file_to_terms.keys(): #dict of tweetids and tweets
			total_score[doc] = self.get_score(doc, qlist)
		return total_score


	def ranked_docs(self):
		ranked_docs = sorted(self.total_score.items(), #.items() returns the key value pair of the dictionary
		key=lambda x: x[1], reverse=True) #key function takes the first index as sorting factor
		return ranked_docs
