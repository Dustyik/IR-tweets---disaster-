...
class BERTmoddel:
    ...
    def return_BERT_query(self, article_id, article_title):
        ...
        topKResults["BERT_similarity"] = topKResults.apply(lambda row: cosine_similarity(np.array(query_embedding).reshape(1, -1), np.array(row.embedding).reshape(1, -1)).item())
