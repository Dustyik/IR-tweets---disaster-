...
class Word2VecModel:
    def __init__(self, tweets_data):
        self.tweets_data = self.process_corpus(tweets_data)
        self.model = self.create_model(self.tweets_data)
        
    def process_corpus(self):
        list_of_list = [row.split(',') for row in self.tweets_data.clean_text]
        return list_of_list
    ...
    def return_most_significant_tweets(query, n):
        ...
        query_result['similarity']=qresult['vector'].apply(lambda x: cosine_similarity(np.array(qvector).reshape(1, -1),np.array(x).reshape(1, -1)).item())
